...
OPEN_GAME = 'hermes/intent/MagiBoxEi2i:Ouvrejeu'
...
def onConnect(client, userData, flags, rc):
    ...
    mqttPublish.single('hermes/feedback/sound/toggleOn', payload=json.dumps({'siteId': 'default'}),
...
def onMessage(client, userData, message):
    ...
    global game, currentStep, confirm
    ...
    if intent == OPEN_GAME:
        ...
        slotNameGame = payload['slots'][0]['value']['value'].encode('utf-8')
        ...
        if os.path.isfile('./games/{}/{}.json'.format(settings.LANG, slotNameGame.lower())):
            endTalk(sessionId, text=lang['confirmOpening'].format(payload['slots'][0]['rawValue']))
            currentStep = 0

            file = codecs.open('./games/{}/{}.json'.format(settings.LANG, slotNameGame.lower()), 'r',
                               encoding='utf-8')
            string = file.read()
            file.close()
            ...
            gameName = recipe['name'] if 'phonetic' not in recipe else recipe['phonetic']
