...
def onMessage(client, userData, message):
	...
	if intent == OPEN_GAME:
		...
		if os.path.isfile('./games/{}/{}.json'.format(settings.LANG, slotRecipeName.lower())):
			endTalk(sessionId, text=lang['confirmOpening'].format(payload['slots'][0]['rawValue']))
			currentStep = 0

			file = codecs.open('./games/{}/{}.json'.format(settings.LANG, slotRecipeName.lower()), 'r', encoding='utf-8')
			string = file.read()
			file.close()
